...
public class UDPInput implements Runnable {
	...
	private static final int HEADER_SIZE = Packet.IP4_HEADER_SIZE
			+ Packet.UDP_HEADER_SIZE;

	private Selector selector;
	private ConcurrentLinkedQueue<ByteBuffer> outputQueue;

	public UDPInput(ConcurrentLinkedQueue<ByteBuffer> outputQueue,
			Selector selector) {
			...
			while (!Thread.interrupted()) {
				...
				while (keyIterator.hasNext() && !Thread.interrupted()) {
					...
					if (key.isValid() && key.isReadable()) {
						...
						DatagramChannel inputChannel = (DatagramChannel) key
								.channel();
						// XXX: We should handle any IOExceptions here
						// immediately,
						...
						referencePacket.updateUDPBuffer(receiveBuffer,
								readBytes);
